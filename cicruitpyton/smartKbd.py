...
class SmartKbd(object):
    ...
    def proceedChars(self,rxdata:str, DEBUG:bool = False  ): 
        ...
        l_char = rxdata
        if rxdata.startswith(chr(27)):
            print('<esc>',rxdata)
            if rxdata.startswith(chr(27)+chr(91)):
        ...
        if l_char =='enter' or ord(rxdata[:1])==10:
            self.grblStateObj.sent2grbl(self.getc())
        elif l_char == ' ' or l_char == 'space' or l_char =='shift space' or l_char =='tab':
            self.put_char(' ')
            self.grblStateObj.neoShowEdit()
        elif l_char == 'backspace' or l_char =='shift backspace' or ord(rxdata[:1])==8:
            self.backspace()
            self.grblStateObj.neoShowEdit()
        elif l_char in ('~','!','?','#','^','@') or \
